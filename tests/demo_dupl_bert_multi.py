...
from transformers import TextClassificationPipeline, BertTokenizer, Trainer, DataCollatorWithPadding, TrainingArguments
...
model = torch.load("/local/scratch/bzhao44/baseline3-models/bert-baseline3-1")
...
for i in range(0, len(pred_res)):
    if pred_res[i] == actl_res[i]:
        ...
        print("The Resume is" + str(sample_dataset['test'][i]))
        print()
